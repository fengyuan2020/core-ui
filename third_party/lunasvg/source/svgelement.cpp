...
static std::string toAsciiLower(std::string_view input)
{
    std::string out;
...
static std::vector<unsigned char> decodeBase64(std::string_view input)
{
    static const unsigned char kInvalid = 255;
    unsigned char table[256];
    std::fill(table, table + 256, kInvalid);
    for(int i = 0; i < 26; ++i) {
...
static Bitmap loadImageResource(const std::string& href)
{
    if(href.compare(0, 5, "data:") == 0) {
        ...
        if(index == std::string_view::npos)
            return Bitmap();
        auto meta = toAsciiLower(input.substr(5, index - 5));
        input.remove_prefix(index + 1);
        if(meta.find("image/svg+xml") != std::string::npos && meta.find(";base64") != std::string::npos) {
            auto decoded = decodeBase64(input);
            if(decoded.empty())
                return Bitmap();
            auto document = Document::loadFromData(reinterpret_cast<const char*>(decoded.data()), decoded.size());
            if(!document)
                return Bitmap();
            auto width = static_cast<int>(std::ceil(document->width()));
            auto height = static_cast<int>(std::ceil(document->height()));
            if(width <= 0 || height <= 0)
                return Bitmap();
...
void SVGSwitchElement::render(SVGRenderState& state) const
{
    if(isDisplayNone())
        return;
    SVGBlendInfo blendInfo(this);
    SVGRenderState newState(this, state, localTransform());
    newState.beginGroup(blendInfo);
    /* <switch> 只渲染第一个可显示的子元素(浏览器语义)。foreignObject 等不支持的
     * 分支已在解析期作为 Unknown 出局, 不会进入 children。 */
    for(const auto& child : children()) {
        if(auto element = toSVGElement(child)) {
            if(!element->isHiddenElement()) {
                element->render(newState);
                break;
            }
        }
    }
...
// --- gh-svg 滤镜元素 (L173 Phase 3) ---------------------------------------

namespace {

float parseFilterNumber(const std::string& s, float fallback)
{
    if(s.empty())
        return fallback;
...
std::vector<float> parseFilterNumberList(const std::string& s)
{
...
SVGFeDropShadowElement::SVGFeDropShadowElement(Document* document)
    : SVGElement(document, ElementID::FeDropShadow)
{
}

float SVGFeDropShadowElement::dx() const
{
    return parseFilterNumber(getAttribute(PropertyID::Dx), 2.f);
}

float SVGFeDropShadowElement::dy() const
{
    return parseFilterNumber(getAttribute(PropertyID::Dy), 2.f);
}

float SVGFeDropShadowElement::stdDeviation() const
{
    // stdDeviation 可为 "x y" 两值; v1 取第一个 (strtof 在空白处停)。
    return std::max(0.f, parseFilterNumber(getAttribute(PropertyID::Std_Deviation), 2.f));
}

Color SVGFeDropShadowElement::floodColor() const
{
    const auto& value = getAttribute(PropertyID::Flood_Color);
    if(value.empty())
        return Color::Black;
    plutovg_color_t color;
    int length = plutovg_color_parse(&color, value.data(), static_cast<int>(value.length()));
    if(length == 0)
        return Color::Black;
    return Color(plutovg_color_to_argb32(&color));
}

float SVGFeDropShadowElement::floodOpacity() const
{
    return std::clamp(parseFilterNumber(getAttribute(PropertyID::Flood_Opacity), 1.f), 0.f, 1.f);
}

SVGFeGaussianBlurElement::SVGFeGaussianBlurElement(Document* document)
    : SVGElement(document, ElementID::FeGaussianBlur)
{
}

float SVGFeGaussianBlurElement::stdDeviation() const
{
    return std::max(0.f, parseFilterNumber(getAttribute(PropertyID::Std_Deviation), 0.f));
}

SVGFeOffsetElement::SVGFeOffsetElement(Document* document)
    : SVGElement(document, ElementID::FeOffset)
{
}

float SVGFeOffsetElement::dx() const
{
    return parseFilterNumber(getAttribute(PropertyID::Dx), 0.f);
}

float SVGFeOffsetElement::dy() const
{
    return parseFilterNumber(getAttribute(PropertyID::Dy), 0.f);
}

SVGFeColorMatrixElement::SVGFeColorMatrixElement(Document* document)
    : SVGElement(document, ElementID::FeColorMatrix)
{
}

bool SVGFeColorMatrixElement::isSourceAlphaInput() const
{
    std::string input = getAttribute(PropertyID::Filter_Input);
    std::transform(input.begin(), input.end(), input.begin(),
                   [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    return input == "sourcealpha";
}

bool SVGFeColorMatrixElement::shadowColor(Color& color, float& opacity) const
{
    auto values = parseFilterNumberList(getAttribute(PropertyID::Values));
    if(values.size() < 20)
        return false;
    color = Color(unitToByte(values[4]), unitToByte(values[9]), unitToByte(values[14]));
    const float alphaMul = values[18];
    const float alphaAdd = values[19];
    opacity = std::clamp(std::abs(alphaMul) > 0.0001f ? alphaMul : alphaAdd,
                         0.f, 1.f);
    return true;
}
...
const SVGFeDropShadowElement* SVGFilterElement::dropShadow() const
{
    for(const auto& child : children()) {
        auto element = toSVGElement(child);
        if(element && element->id() == ElementID::FeDropShadow)
            return static_cast<const SVGFeDropShadowElement*>(element);
    }
    return nullptr;
}

SVGFilterEffect SVGFilterElement::effect() const
{
    if(auto ds = dropShadow()) {
        SVGFilterEffect effect;
        effect.type = SVGFilterEffectType::RecolorSourceAlpha;
        effect.dx = ds->dx();
        effect.dy = ds->dy();
        effect.stdDeviation = ds->stdDeviation();
        effect.color = ds->floodColor();
        effect.opacity = ds->floodOpacity();
        return effect;
    }

    const SVGFeGaussianBlurElement* blur = nullptr;
    const SVGFeOffsetElement* offset = nullptr;
    bool sourceAlpha = false;
    bool haveShadowColor = false;
    ...
    for(const auto& child : children()) {
        ...
        if(!element)
            continue;
        if(element->id() == ElementID::FeGaussianBlur && blur == nullptr) {
            blur = static_cast<const SVGFeGaussianBlurElement*>(element);
        } else if(element->id() == ElementID::FeOffset) {
            offset = static_cast<const SVGFeOffsetElement*>(element);
        } else if(element->id() == ElementID::FeColorMatrix) {
            auto matrix = static_cast<const SVGFeColorMatrixElement*>(element);
            sourceAlpha = sourceAlpha || matrix->isSourceAlphaInput();
            if(matrix->shadowColor(shadowColor, shadowOpacity))
                haveShadowColor = true;
        }
    }
    ...
    SVGFilterEffect effect;
    effect.type = (sourceAlpha || haveShadowColor)
        ? SVGFilterEffectType::RecolorSourceAlpha
        : SVGFilterEffectType::SourcePaintBlur;
    effect.stdDeviation = blur->stdDeviation();
    if(offset) {
        effect.dx = offset->dx();
        effect.dy = offset->dy();
    }
    if(effect.type == SVGFilterEffectType::RecolorSourceAlpha) {
        effect.color = shadowColor;
        effect.opacity = shadowOpacity;
    }
    return effect;
}
