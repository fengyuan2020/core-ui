...
/* <switch>: 条件渲染容器。按浏览器语义只渲染第一个可显示的子元素。lunasvg 不支持
 * foreignObject(Extensibility), 它在解析期作为 Unknown 被丢弃, 故 drawio/mxgraph 的
 * "<foreignObject> + <image> 降级" 在此自然落到 <image> 分支。完整的 requiredFeatures
 * /systemLanguage 求值未实现, 简化为"首个可显示子元素"。 */
...
// gh-svg (L173 Phase 3): SVG 滤镜 — 自研叠加层。LunaSVG 上游不支持 <filter>;
// gh-svg 把 filter primitives 接进 DOM, 在 beginGroup/endGroup 离屏合成钩子里
// 对离屏 surface 应用 (见 svgfilter.{h,cc} + svgrenderstate.cpp)。当前覆盖
// feDropShadow、blur-only feGaussianBlur, 以及常见 SourceAlpha 阴影链。
// 这些元素不渲染 (base render no-op, 且 isHiddenElement), 仅被 filter="url(#id)" 引用。
enum class SVGFilterEffectType : uint8_t {
    None,
    SourcePaintBlur,
    RecolorSourceAlpha
};

struct SVGFilterEffect {
    SVGFilterEffectType type = SVGFilterEffectType::None;
    float dx = 0.f;
    float dy = 0.f;
...
public:
    ...
    // 从属性解析 (dx/dy/stdDeviation/flood-color/flood-opacity)。缺省按 SVG 规范。
    float dx() const;
    float dy() const;
...
class SVGFeColorMatrixElement final : public SVGElement {
public:
    SVGFeColorMatrixElement(Document* document);
    bool isSourceAlphaInput() const;
    bool shadowColor(Color& color, float& opacity) const;
};
...
public:
    ...
    // 第一个 <feDropShadow> 子 (v1 只认它)。无则 nullptr → 该 filter 无可见效果。
    const SVGFeDropShadowElement* dropShadow() const;
    SVGFilterEffect effect() const;
};
