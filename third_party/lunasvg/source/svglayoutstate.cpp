...
/* CSS 颜色函数前缀匹配: input 是否以 funcName( ... ) 起头。匹配则把括号内顶层
 * 逗号切分(尊重嵌套括号, 各参去首尾空白)填进 args, consumed 置为整段函数(含函数
 * 名与右括号)的字节数; input 本身不动, 由调用方决定是否消费。括号不闭合返 false。
 * plutovg 不识别 light-dark()/var() 等 CSS Color-4/5 函数 → 在此先行解析。 */
static bool matchColorFunction(std::string_view input, std::string_view funcName,
                               std::vector<std::string_view>& args, size_t& consumed)
{
    std::string_view s = input;
    if(!skipString(s, funcName))   /* 大小写敏感; drawio/figma 均输出小写函数名 */
        return false;
    skipOptionalSpaces(s);
    if(s.empty() || s.front() != '(')
        return false;
    args.clear();
    int depth = 0;
    size_t argStart = 1;           /* s[0] == '(' */
    for(size_t i = 0; i < s.size(); ++i) {
        ...
        if(c == '(') {
            ...
            if(--depth == 0) {
                std::string_view a = s.substr(argStart, i - argStart);
                stripLeadingAndTrailingSpaces(a);
                args.push_back(a);
                consumed = (input.size() - s.size()) + i + 1;
                return true;
            }
        } else if(c == ',' && depth == 1) {
            std::string_view a = s.substr(argStart, i - argStart);
...
static std::optional<Color> parseColorValue(std::string_view& input, const SVGLayoutState* state)
{
    ...
    /* CSS Color-4: light-dark(<light>, <dark>) — 我们按 light-mode 栅格化(文档意图
     * 为浅色, 参考输出亦浅色), 取第一参(浅色分支), 递归解析以兼容 hex/rgb/var 嵌套。 */
    std::vector<std::string_view> args;
    size_t consumed = 0;
    if(matchColorFunction(input, "light-dark", args, consumed) && !args.empty()) {
        std::string_view light = args.front();
        auto color = parseColorValue(light, state);
    ...
    /* CSS var(--name, <fallback>) — 无自定义属性解析能力, 取兜底值(第二参); 缺则失败
     * 落默认色(行为不变)。drawio 的 var 多嵌在 light-dark 暗色分支内, 几不单独命中。 */
    if(matchColorFunction(input, "var", args, consumed) && args.size() >= 2) {
        std::string_view fallback = args[1];
        auto color = parseColorValue(fallback, state);
        if(color && fallback.empty()) {
            input.remove_prefix(consumed);
            return color;
        }
        return std::nullopt;
